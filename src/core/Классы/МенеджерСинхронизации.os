...
Процедура СинхронизироватьХранилищеКонфигурацийСГит(Знач КаталогРабочейКопии, 
		...
		Знач Лимит = 0,
		Знач ПараметрыДоступаКХранилищу) Экспорт
	...
	Пока СледующаяВерсия <= МаксимальнаяВерсияДляРазбора Цикл
		...
		Если СтрокаВерсии <> Неопределено Тогда
			...
			Попытка
				...
				Лог.Отладка("Начало процедуры РазложитьМодулиПоНомеруВерсииХранилища1С: " + ТекущаяДата());				
				РазложитьМодулиПоНомеруВерсииХранилища1С(КаталогРабочейКопии, ?(ПараметрыДоступаКХранилищу=Неопределено, ФайлХранилища, ПутьКХранилищу), СледующаяВерсия, Формат, ПараметрыДоступаКХранилищу);
				Лог.Отладка("Окончание процедуры РазложитьМодулиПоНомеруВерсииХранилища1С: " + ТекущаяДата());
...
Процедура РазложитьМодулиПоНомеруВерсииХранилища1С(Знач КаталогРабочейКопии, Знач ФайлХранилища, Знач НомерВерсии, Знач Формат, ПараметрыДоступаКХранилищу) Экспорт  

	ФайлХранилища = Новый Файл(ФайлХранилища);
	
	// выгрузка средствами tool1cd
	...
	// выгрузка штатными средствами конфигуратора	
	ИначеЕсли ФайлХранилища.Существует() И ФайлХранилища.ЭтоКаталог() Тогда
	
		Лог.Отладка("Получаем версию хранилища " + НомерВерсии);		
		Попытка
			РазобратьФайлКонфигурацииШтатнымиСредствами(ФайлХранилища.ПолноеИмя, НомерВерсии, ПараметрыДоступаКХранилищу, КаталогРабочейКопии, Формат);
		Исключение
			ВызватьИсключение;
		КонецПопытки;
...
УдалятьВременныеФайлы = Истина; // по умолчанию временные файлы удаляются
